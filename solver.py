...
class Solver:
    ...
    def getController(self):
        ...
        self.controller: BaseController_Seg_GSFVI = self.config[name]['controller'](
            self.net)
    ...
    def getTrainDataloader(self):
        ...
        self.train_dataloader = torch.utils.data.DataLoader(
            ...
            collate_fn=CollateGPU(transforms=transforms.Compose([
                ...
                CentralCropTensor((200, 200), (128, 128)),#(128, 128)
    ...
    def getValidationDataloader(self):
        ...
        #     num_workers=0,
        #     collate_fn=CollateGPU(transforms=transforms.Compose([
        #         CentralCropTensor((200, 200), (96, 96)),
        #     ])),
        #     pin_memory=False)
    ...
    def test(self):
        ...
        epoch = self.config['Test']['epoch']

        if epoch == 'best':
            results = {}
            for checkpoint_epoch, name_suffix in (
                ('best', 'best'),
                ('best_test', 'best_test'),
            ):
                checkpoint_path = os.path.join(
                    self.model_save_path, '{}.pt'.format(checkpoint_epoch))
                if not os.path.exists(checkpoint_path):
                    print('Skip {}: checkpoint not found at {}'.format(
                        checkpoint_epoch, checkpoint_path))
                    continue

                self.loadCheckpoint(self.controller.net, checkpoint_epoch)
                result_name = '{}-{}'.format(self.model_save_name, name_suffix)
                results[checkpoint_epoch] = self.controller.test(
        ...
        self.loadCheckpoint(self.controller.net, epoch)
