...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, pair_list_path, pair_dir, transform=None):
        self.pair_list = np.loadtxt(pair_list_path).astype(np.int64)
    ...
    def preload(self):
        ...
        for c_no, s, ed, es in self.pair_list:
            ...
            if s not in self.dataset[c_no]:
                self.dataset[c_no][s] = {
                    ed: {
                        'img': torch.tensor(normalize(ed_unit['img'].astype(np.float64))).unsqueeze(0).unsqueeze(0).float(),
                        'seg': torch.tensor(ed_unit['seg'].astype(np.float64)).unsqueeze(0).unsqueeze(0).float(),
                    },
                    es: {
                        'img': torch.tensor(normalize(es_unit['img'].astype(np.float64))).unsqueeze(0).unsqueeze(0).float(),
                        'seg': torch.tensor(es_unit['seg'].astype(np.float64)).unsqueeze(0).unsqueeze(0).float()
                    },
                }
            else:
...
class Dataset_without_label(torch.utils.data.Dataset):
    def __init__(self, pair_list_path, pair_dir, transform=None):
        self.pair_list = np.loadtxt(pair_list_path).astype(np.int64)
        self.transform = transform
        self.pair_dir = pair_dir
