...
class MetricTest:
    ...
    def testFlow(self, flows_t, flowt_s, case_no):
        ...
        # bes_t_result = self.be_estimate(flows_t[:, [1,0], ...]).cpu().numpy()
    ...
    def _calc_box_stats(self, values):
    ...
    def beBoxStatsByDataset(self, direction='ed_to_es'):
        self.be_box_stats_by_dataset = {}

        for dataset_name in ['York', 'ACDC', 'MICCAI']:
            dataset = self.details.get(dataset_name, {})
            case_mean = [
                np.mean(dataset[case_no]['flow'][direction]['BE'])
                for case_no in dataset if 'flow' in dataset[case_no]
            ]
            self.be_box_stats_by_dataset[dataset_name] = self._calc_box_stats(
                case_mean)

        return self.be_box_stats_by_dataset

    def saveBEBoxStatsCsv(self, network, name, save_path, direction='ed_to_es'):
        os.makedirs(save_path, exist_ok=True)
        be_box_stats = self.beBoxStatsByDataset(direction)
        file_name = '%s-%s-%s-BE-boxstats-%s.csv' % (
            network, name, direction, time.strftime('%Y%m%d%H%M%S'))
        csv_save_path = os.path.join(save_path, file_name)

        with open(csv_save_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(
                ['dataset', 'direction', 'metric', 'min', 'q1', 'median',
                 'mean', 'q3', 'max'])
            for dataset_name in ['York', 'ACDC', 'MICCAI']:
                stat_dict = be_box_stats[dataset_name]
                writer.writerow([
                    dataset_name, direction, 'BE', stat_dict['min'],
                    stat_dict['q1'], stat_dict['median'], stat_dict['mean'],
                    stat_dict['q3'], stat_dict['max']
                ])

        return csv_save_path

    def outputBEBoxStats(self, direction='ed_to_es'):
        be_box_stats = self.beBoxStatsByDataset(direction)
        print('BE box statistics for %s' % direction)
        for dataset_name in ['York', 'ACDC', 'MICCAI']:
            print(dataset_name, be_box_stats[dataset_name])
        return be_box_stats
